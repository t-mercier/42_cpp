...
/*   Updated: 2023/06/29 21:53:14 by tmercier      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
Fixed::Fixed(const Fixed &n) : _raw(n._raw) { std::cout << "Copy constructor called" << std::endl; }
...
float         Fixed::toFloat() const { return ((float)getRawBits()) / (float)(1 << _fract); }

std::ostream &operator<<(std::ostream &o, const Fixed &f) { return o << f.toFloat(); }

bool          Fixed::operator>(const Fixed &b) const { return (getRawBits() > b.getRawBits()); }
bool          Fixed::operator==(const Fixed &b) const { return (getRawBits() == b.getRawBits()); }
